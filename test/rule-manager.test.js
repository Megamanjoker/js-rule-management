...
describe("This test runs through multiple input scenarios for the push feature", () => {
  ["input", {}, 1144].forEach((testData) => {
    test(
      "Rule Manager should treat all none rule objects handed to it as an input of "
        .concat(JSON.stringify(testData))
        .concat(" to the a rule object that has the script of ()=>{}"),
      () => {
        const ruleManager = new RuleManager();
  ...
  [
    ...
    test(
      "Rule Manager should push multiple rule objects "
        .concat(JSON.stringify(testData))
        .concat(" and add them to the rule set"),
      () => {
        const ruleManager = new RuleManager();
        const input = testData;
        ruleManager.push(...input);
  ...
  [
    ...
    test(
      "Rule Manager should flatten an array of rule objects "
        .concat(JSON.stringify(testData))
        .concat(" and add them to the rule set"),
      () => {
        const ruleManager = new RuleManager();
  ...
  [
    ...
    test(
      "Rule Manager should work with this input".concat(
  ...
  [[new Rule(), {}], [new Rule()], [new Rule(), "asdf", {}]].forEach(
    (testData) => {
      test(
        "Rule Manager should work with this input "
          .concat(JSON.stringify(testData))
          .concat(" and return a length of ".concat("" + testData.length)),
        () => {
          const ruleManager = new RuleManager();
          expect(ruleManager.push(testData)).toEqual(testData.length);
        }
      );
    }
  );
...
describe("This is to test the running all the rules", () => {
  [1, 2, 5, 54].forEach((testData) => {
    test(
      "After adding " +
        testData +
        " rule(s) to the rule manager, called runRules should call all the rules in the array " +
        testData +
        " time(s)",
      () => {
  ...
  ["help", "me", "I", "am", "in", "a", "tower", "of", "pizza", 14452].forEach(
    (testData) => {
      test(
        "After adding a rule with the input " +
          testData +
          ", when runRules should call all the inputs should be give to the script",
        () => {
          const mockScript = jest.fn(() => {});
  ...
  [
    ...
    test(
      "Given an array of rules with length " +
        testData.length +
        " that will error on the scripts, do not throw",
      () => {
  ...
  [
    ...
    test(
      index +
        " - Given an array of rules, when one of the scripts could error, then store all the results in the object",
      () => {
        const input = testData.map((value) => new Rule(undefined, value));
        const ruleManager = new RuleManager();
  ...
  [
    ...
    test(
      ...
      () => {
        ...
        ruleManager.push(input);
        expect(ruleManager.runRules()).toEqual(expectedOutput);
      }
    );
  });
});
